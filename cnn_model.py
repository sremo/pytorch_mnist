...
class Model(nn.Module):
    def __init__(self,params):
        ...
        # how often (in the number of batches) to print loss and save the model during training
    ...
    def train_model(self,epoch,optimizer,train_loader):
        """
    ...
    def default_optimizer(self, learning_rate = 0.005, momentum=0.5):
        return optim.SGD(self.parameters(), learning_rate,momentum)

    def run_train_and_eval(self, optimizer, n_epochs, test_loader, train_loader):
        """
        input: optimizer
        ...
        for epoch in range(1, n_epochs + 1):
            self.train_model(epoch, optimizer, train_loader)
            test_metrics = self.eval_model(test_loader)
        return test_metrics
...
def hyperpameter_tuning(test_loader, train_loader, parameters, metric, maximize = True):
    ...
    for parameters in parameters_list:
        nm = Model(parameters)
        optimizer = nm.default_optimizer(parameters['learning_rate'],parameters['momentum'])
        metrics = nm.run_train_and_eval(optimizer, 2, test_loader, train_loader)
